...
def search(text: str) -> SearchResult:
    ...
    dres_logger("EAGLE", "SEMANTIC SEARCH").info(
        "NOT VERIFIED" if not session_info else session_info.username, 
        f"Search for {text}")
...
def fetch_video(name: str):
    ...
    dres_logger("EAGLE", "VIEW CHANGE").info(
        "NOT VERIFIED" if not session_info else session_info.username, 
        f"Fetch sequence for {image_id}")
...
def search_image(name: str) -> SearchResult:
    ...
    dres_logger("EAGLE", "SEARCH").info(
        "NOT VERIFIED" if not session_info else session_info.username, 
        f"Visual similarity search for {image_id}")
...
def dres_track(name: str):
    dres_logger("EAGLE", "ENV").info(
        "NOT VERIFIED" if not session_info else session_info.username, 
        f"Set default DRES task to {name}")
